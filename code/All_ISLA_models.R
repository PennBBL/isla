#' ---
#' title: "Multivariate Voxelwise `gam()` For All ISLA Models"
...
system(run_command)
#' ***
#' 3. rawCBF ~ s(age) +  s(age, by=sex) + sex + cbfMotion
demographics <-
  read.csv("/data/joy/BBL/studies/pnc/n1601_dataFreeze/demographics/n1601_demographics_go1_20161212.csv") %>%
  as_tibble() %>%
  mutate(sex = as.ordered(as.factor(sex)),
...
#' ***
#' 4. rawCBF ~ s(age) +  s(age, by=sex) + sex + cbfMotion + GMDvox
demographics <-
  read.csv("/data/joy/BBL/studies/pnc/n1601_dataFreeze/demographics/n1601_demographics_go1_20161212.csv") %>%
  as_tibble() %>%
  mutate(sex = as.ordered(as.factor(sex)),
...
gmd <-
  list.files(gmd_path,  pattern = ".nii.gz", recursive = TRUE, full.names = TRUE) %>%
  tibble(GMDVox = .) %>%
  mutate(scanid = str_extract(GMDVox, "[:digit:]{4,}")) %>%
  select(scanid, everything())
...
inclusion <- "include"
subjID <- "scanid"
my_formula <- "~s(age)+s(age,by=sex)+sex+pcaslRelMeanRMSMotion+GMDVox"
padjust <- "fdr"
run_command <- sprintf("Rscript /data/jux/BBL/projects/isla/code/voxelwiseWrappers/gam_voxelwise.R -c %s -o %s -p %s -m %s -s %s -i %s -u %s -f %s -a %s -n 5 -s 0 -k 10", covariates, output, image_paths, mask, smoothing, inclusion, subjID, my_formula, padjust)
